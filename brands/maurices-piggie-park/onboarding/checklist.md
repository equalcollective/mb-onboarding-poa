# Onboarding Checklist: Maurice's Piggie Park

> **Account Manager:** Aryan
> **Started:** TBD

---

## Access & Permissions

- [ ] Seller Central access obtained
- [ ] Advertising Console access obtained
- [ ] Brand Registry access confirmed
- [ ] A+ Content permissions verified
- [ ] Brand Analytics access confirmed

## Client Data Collection

- [ ] Cost of Goods (COGS) data received from client
- [ ] Design assets received from client
- [ ] Brand guidelines received
- [ ] Product images/creatives received
- [ ] Drive/folder links organized

## Client Understanding

- [ ] End customer understanding documented
- [ ] Client goals clearly defined
- [ ] Success metrics agreed upon
- [ ] Communication cadence established
- [ ] Key contacts identified

## Basic Operations Check

- [ ] Asset structure audit completed (images, A+, video)
- [ ] Buy Box analysis done
- [ ] Pricing check completed
- [ ] Listing quality audit done
- [ ] Backend keywords reviewed

## Report Analysis

- [ ] SQP Analysis completed
- [ ] Data Dive analysis completed
- [ ] Business Report analysis completed
- [ ] Ad Report analysis completed
- [ ] Competitor analysis completed
- [ ] Inventory analysis completed

## Product Selection & Planning

- [ ] Priority products identified
- [ ] Product briefs created for priority products
- [ ] Product-level onboarding docs created

## Action Planning

- [ ] Account-level POA created
- [ ] Product-level POAs created
- [ ] Actionables decided and documented
- [ ] Timeline established

## Tracking & Monitoring Setup

- [ ] Rank tracking set up (Rank Radar / equivalent)
- [ ] Price tracking set up (Price Check / equivalent)
- [ ] Competitor monitoring configured
- [ ] Review monitoring configured

## Final Review

- [ ] All documentation reviewed for completeness
- [ ] Handoff notes prepared (if applicable)
- [ ] First weekly action plan ready
- [ ] Client kick-off/review meeting scheduled

---

## Notes

- Start date to be confirmed with client

---

*Last Updated: 2026-01-27 by Aryan*
